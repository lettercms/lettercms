...
export default function MetaInputs({categories, onFocus, onBlur, onChange}) {
  ...
  return <div className={metaInputs}>
    <Input label='Enlace' type='url' onChange={({target: {value}}) => setData('url', value)} onFocus={onFocus} onBlur={onBlur} value={data.url}/>
    <Input label='Descripción' type='textarea' onChange={({target: {value}}) => setData('description', value)} onFocus={onFocus} onBlur={onBlur} value={data.description}/>
    {
      categories?.length > 0 &&

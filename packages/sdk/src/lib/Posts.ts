...
class Posts {
  ...
  async single(year: string, month?: StringOrData, date?: StringOrData, url?: StringOrData, data?: PostRequestOptions): Promise<PostResponse> {
    const basePath = '/post/';
    let path = '';
    ...
    if (!month) //Only URL
      ...
      if (typeof month === 'object') { //URL + Data
        path = basePath + year;

        data = month;
      } 
      else if (typeof month === 'string') //Category + URL
        path = `${basePath}${year}/${month}`;
    }

    else if (date && !url) {
      if (typeof date === 'object') { //Category + URL + Data
        path = `${basePath}${year}/${month}`;
        data = date;
      } 
      else if (typeof date === 'string') //Year + Month + URL
        path = `${basePath}${year}/${month}/${date}`;
    }

    else if (url && !data) {
      if (typeof url === 'object') { //Year + Month + URL + Data
        path = `${basePath}${year}/${month}/${date}`;
        data = url;
      } 
      else if (typeof url === 'string') //Year + Month + Date + URL
        path = `${basePath}${year}/${month}/${date}/${url}`;
    } else {
      if (typeof data !== 'object')
        throw new TypeError('Data must be typeof Object');
      else //Year + Month + Date + URL + Data
        path = `${basePath}${year}/${month}/${date}/${url}`;
    }
  ...
  async search(q: string, data?: PostRequestOptions): Promise<PostsListResponse> {
